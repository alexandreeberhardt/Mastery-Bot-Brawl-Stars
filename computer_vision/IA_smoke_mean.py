...
def load_images(path_to_gameplay):
    ...
    smoke = cv2.imread('/home/alexandre/Mastery-Bot-Brawl-Stars/screens/smoke_small_cropped.png', cv2.IMREAD_UNCHANGED)
...
def get_smoke_info(path_to_gameplay):
    gamescreen, smoke = load_images(path_to_gameplay)
...
def main():
    path_to_gameplay = '../play_screen/screenshot_a2_s6.png'

    gamescreen, smoke = load_images(path_to_gameplay)
    result = find_smokees(gamescreen, smoke)
    rectangles = draw_rectangles(gamescreen, result, smoke)
    marked_screen, _= mark_smokees(gamescreen, rectangles)
    print(get_smoke_info(path_to_gameplay))
